#-------------------------------------------------------------------------------
# Name:        SocialServices: Bed Deallocation
# Purpose:      Deallocates a bed in the beds database, and updates it in the user database
#
# Author:      Stephen
#
# Created:     02/12/2014
...
#initialize database
    ...
    while True:
        ...
        #already deallocated
        if 0 == bedId:
            #show message indicating already checked out
            print("You are already signed out, thanks!")
            #wait 5 sec
            time.sleep(5)
            #continue loop
            continue
        ...
        print("You are registered to bed " + str(bedId) + ", signing you out.")
        #find bed in beds database
        beds_cur = beds_db.find({"_id":bedId})
        #if bed can't be found, report error and continue
        if 0 == beds_cur.count():
            #report error
            print("The bed you reserved (" + str(bedId) + ") was not found in the database, please see administrator")
            #wait 5 sec
            time.sleep(5)
        #bed found
        else:
            #update bed to available
            beds_db.update({"_id":bedId},{"_id":bedId, 'status':"available"})
        #update user bedId to 0
        user_to_upd = tagNo_cur[0].copy()
        user_to_upd["bedId"] = 0
        users_db.update({"_id":user_to_upd["_id"]},user_to_upd)
        #display confirmation of completion
        print("You have been successfully checked out of bed#" + str(bedId))
        #wait 5 sec
        time.sleep(5)
        continue
    pass
